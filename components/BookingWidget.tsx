...
export const BookingWidget: React.FC<BookingWidgetProps> = ({ onComplete }) => {
  ...
  const renderStep1 = () => (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* Hourly Charter Duration */}
      {bookingType === BookingType.HOURLY && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-zinc-400 text-sm mb-2">Hours per Day</label>
            <Input type="number" min="1" max="24" value={request.durationHours || 3} onChange={e => setRequest({ ...request, durationHours: parseInt(e.target.value) || 3 })} />
          </div>
          <div>
            <label className="block text-zinc-400 text-sm mb-2">Number of Days</label>
            <Input type="number" min="1" max="30" value={request.durationDays || 1} onChange={e => setRequest({ ...request, durationDays: parseInt(e.target.value) || 1 })} />
          </div>
        </div>
      )}
